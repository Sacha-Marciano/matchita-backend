...
def classify_document(title: str, text: str, folders: list[str] , tags: list[str] ):
    prompt = (
        ...
        f"- 4 to 6 tags\n"
        f"- A teaser/ short description of the doc\n\n"
        f" Here is a list of existing folders that you may use if a folders matches for the following document : \n"
        f"{folders}\n\n"
        f"Only use the existing folders if they match, otherwise create new ones.\n"
        f"Here is a list of existing tags that you may use if a tag matches for the following document : \n"
        f"{tags}\n\n"
        f"Only use the existing tags if they match, otherwise create new ones.Be very specific when creating new tags\n"
        f"If names are mentioned, include them in the title and tags.\n"
